    ...
    return (
        <>
            <div className='text-center'>
                <label className='text-2xl font-bold'>{props.playerName}</label>
            </div>
            {props.playerCards.length !== 0 && 
            
               <div className="flex flex-row">
                    <img className="w-60 p-5" src={`/images/${props.playerCards?.card1?.card}.PNG`} alt="assasin"/>
                    <img className="w-60 p-5" src={`/images/${props.playerCards?.card2?.card}.PNG`} alt="captain" />
                </div>  
